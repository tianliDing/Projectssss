...
class KDTree
{
  ...
  public:
     ...
     * by operator< in the poinnewt class). /////////////////////In the first step, you must
  ...
  private:
    ...
    int partition(int left, int right, int d, int k);
    void quickSelect(int left, int right, int d);
    void KDTreebuilder(KDTreeNode* subroot, int left, int right);
    Point<Dim> findNearestNeighborhelper(KDTreeNode* subroot,const Point<Dim>& query, int d) const;
    void select(int left, int right, int d, int mid);
